...
namespace Data.cs.Commands
{
    public class ZonasRepositorio : IZonasRepositorio
    {
        ...
        public async Task<Response<List<EntZonas>>> DGetByName(string sNombre)
        {
            var response = new Response<List<EntZonas>>();
            try
            {
                var items = await dbContext.Zonas.AsNoTracking().Where(x => x.bEliminado == false && x.sNombre.ToLower() == sNombre.ToLower()).ToListAsync();
                if (items.Count > 0)
